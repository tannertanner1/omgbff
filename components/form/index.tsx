          ...
          <Card className={cn("w-full max-w-[30rem] border-0")}>
            ...
            <form action={formAction}>
              <CardContent className="flex flex-col gap-6">
                {fields.map(({ name, label, type = "text", required }) => (
                  <div key={name} className="grid gap-2">
                    <Label
                      htmlFor={name}
                      className={required ? "after:ml-0.5 after:text-[#DB4437] after:content-['*']" : ""}
                    >
                      {label}
                    </Label>
                    {type === "textarea" ? (
              ...
              <CardFooter className="flex flex-col gap-4">
                ...
                >
                  {isPending ? <IconLoader className="h-4 w-4 animate-spin motion-reduce:hidden" /> : button}
                </Button>
              </CardFooter>
            </form>
          </Card>
          {state?.message && (
            <div className="mx-auto mt-7 w-full max-w-[30rem] px-6">
              <Alert
                className={cn(
                  "w-full",
                  state.success ? "border-[#0F9D58] text-[#0F9D58]" : "border-[#DB4437] text-[#DB4437]",
                )}
              >
                <div className="flex items-start gap-2">
                  ...
                  <AlertDescription className={cn("w-full", state.success ? "text-[#0F9D58]" : "text-[#DB4437]")}>
                    {state.message}
                  </AlertDescription>
                </div>
              </Alert>
